...
pub fn cast_ray(scene: &Scene, orig: Vector3, dir: Vector3, depth: i32) -> Vector3 {
    cast_ray_with_separate_depths(scene, orig, dir, depth, 0, 0, MAX_DEPTH, MAX_REFLECTION_DEPTH, MAX_REFRACTION_DEPTH)
}
...
pub fn cast_ray_with_params(
    ...
    cast_ray_with_separate_depths(scene, orig, dir, depth, reflection_depth, refraction_depth, max_depth, max_reflection_depth, max_refraction_depth)
}
...
fn cast_ray_with_separate_depths(
    ...
    let reflect_color = if material.albedo[2] > 0.0 && reflection_depth < max_reflection_depth {
        let color = cast_ray_with_separate_depths(scene, point, reflect_dir, depth + 1, reflection_depth + 1, refraction_depth, max_depth, max_reflection_depth, max_refraction_depth);
        color
    } else {
        Vector3::ZERO
    };
    ...
    let refract_color = if material.albedo[3] > 0.0 && refraction_depth < max_refraction_depth {
        cast_ray_with_separate_depths(scene, point, refract_dir, depth + 1, reflection_depth, refraction_depth + 1, max_depth, max_reflection_depth, max_refraction_depth)
    } else {
        Vector3::ZERO
    };
    ...
    for light in &scene.lights {
        ...
        if shadow_intersection.hit && (shadow_intersection.point - point).norm() < (light.position - point).norm() {
            continue;
        }
        
        diffuse_light_intensity += 0.0_f32.max(light_dir.dot(normal));
        specular_light_intensity += 0.0_f32.max((-reflect(-light_dir, normal)).dot(dir))
