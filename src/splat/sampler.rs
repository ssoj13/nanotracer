...
use super::{SurfaceSample, Gaussian, SplatConfig};
use super::sh::{fit_sh, fibonacci_hemisphere};
...
fn sample_sh_radiance(
    ...
    for local_dir in local_dirs {
        ...
        // The VIEW direction in 3DGS convention points FROM camera TO surface
        // So if we're looking at the surface from direction `world_outgoing`,
        // the view direction is `-world_outgoing`
        let view_dir = -world_outgoing;
        
        // Trace ray FROM the surface point IN the outgoing direction
        // to see what radiance arrives from that direction
...
fn refract(i: Vec3, n: Vec3, eta_t: f32, eta_i: f32) -> Vec3 {
    ...
    if k < 0.0 {
        Vec3::X // Total internal reflection fallback
    } else {
...
pub fn generate_splats(scene: &Scene, config: &SplatConfig) -> Vec<Gaussian> {
    ...
    // Use override scale or auto-calculate with reduced overlap (1.2 for sharper results)
    let splat_scale = config.scale_override.unwrap_or_else(|| estimate_scale(config.density, 1.2));
    println!("Splat scale: {:.4} (log: {:.4})", splat_scale, splat_scale.ln());
    
    println!("Fitting SH coefficients ({} samples per splat, degree {})...", 
    ...
    let gaussians: Vec<Gaussian> = samples
        ...
        .map(|(i, sample)| {
            ...
            Gaussian::from_sample(sample, &sh, splat_scale)
        })
        .collect();
